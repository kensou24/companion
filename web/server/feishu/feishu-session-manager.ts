...
// Manages Feishu user session mappings, persistence, and state.
// Modeled after wechat-session-manager.ts for the Feishu (Lark) platform.
...
export class FeishuSessionManager {
  private userSessions = new Map<string, FeishuUserSession>();
  private persistPath: string;
  private userIdBySession = new Map<string, string>();
  private persistTimer: ReturnType<typeof setTimeout> | null = null;

  constructor(opts: FeishuSessionManagerOptions) {
  ...
  getOrCreateUserSession(userId: string): FeishuUserSession {
  ...
  getUserSession(userId: string): FeishuUserSession | undefined {
    return this.userSessions.get(userId);
  }

  // ─── Reverse mapping (sessionId → userId) ─────────────────────────────
  ...
  persist(): void {
    // Debounced: coalesce rapid writes within 2s
  ...
  /** Force-write to disk immediately (used on shutdown) */
  flush(): void {
    try {
      ...
      console.error("[feishu-session] Failed to persist:", err);
    }
  }
  ...
  private restore(): void {
    try {
      ...
      console.log(`[feishu-session] Restored ${this.userSessions.size} user session mappings`);
    } catch (err) {
      console.error("[feishu-session] Failed to restore:", err);
    }
  }
}
