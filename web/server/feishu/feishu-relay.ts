...
// Feishu users via the send queue. Adapted from wechat-relay.ts but uses
// chatId-based routing (P2P chat or group chat) instead of userId.
...
import { formatSessionName, formatSingleQuestion } from "./feishu-command-handler.js";
import type { WsBridge } from "../ws-bridge.js";
import type { BrowserIncomingMessage } from "../session-types.js";
import type { FeishuUserSession, FeishuRelayData, PendingPermission, PendingAskQuestion } from "./types.js";
import { FeishuSessionManager } from "./feishu-session-manager.js";
import { FeishuSendQueue } from "./feishu-send-queue.js";

// Progress heartbeat timing
const HEARTBEAT_INITIAL_DELAY_MS = 30_000;
...
function extractToolResults(msg: BrowserIncomingMessage): Array<{ tool_use_id: string; content: string; is_error: boolean }> {
  if (msg.type !== "assistant") return [];
  const raw = msg as Record<string, unknown>;
  const message = raw.message;
  if (!message || typeof message !== "object") return [];
...
export interface FeishuRelayDeps {
  wsBridge: WsBridge;
  sessionManager: FeishuSessionManager;
  sendQueue: FeishuSendQueue;
  sendTyping: (chatId: string) => Promise<void>;
}

export class Relay {
  private deps: FeishuRelayDeps;
  private sessionCleanups = new Map<string, Array<() => void>>();
  private sessionRelayData = new Map<string, FeishuRelayData>();
  // Track chatId per session so we can route messages to the right Feishu chat
  private sessionChatIds = new Map<string, string>();

  constructor(deps: FeishuRelayDeps) {
    this.deps = deps;
  }
  ...
  getRelayData(sessionId: string): FeishuRelayData | undefined {
    return this.sessionRelayData.get(sessionId);
  }

  /** Get the Feishu chatId associated with a session relay. */
  getChatId(sessionId: string): string | undefined {
    return this.sessionChatIds.get(sessionId);
  }

  initRelayData(sessionId: string): FeishuRelayData {
    const data: FeishuRelayData = {
      pendingText: "",
      lastTypingTs: 0,
  ...
  ensureRelay(sessionId: string, userId: string, chatId: string): void {
    ...
    const relaySend = (text: string) => {
      const cid = this.sessionChatIds.get(sessionId) ?? chatId;
      sendQueue.enqueue(cid, text);
    ...
    const unsubAssistant = companionBus.on("message:assistant", ({ sessionId: sid, message }) => {
          ...
          if (verboseMode) {
            ...
            if (relayData) {
              ...
              if (!relayData.toolNotifyTimer) {
                relayData.toolNotifyTimer = setTimeout(() => this.flushToolNotifyBuffer(sessionId), 15_000);
              }
            }
          }
        }
      ...
      // Tool result previews (verbose mode only)
    ...
    const unsubPermReq = companionBus.on("session:permission-request", ({ sessionId: sid, request }) => {
      if (sid !== sessionId) return;
      this.handlePermissionRequest(sessionId, userId, chatId, request);
    });
  ...
  flushToolNotifyBuffer(sessionId: string): void {
    const relayData = this.sessionRelayData.get(sessionId);
    if (!relayData || relayData.toolNotifyBuffer.length === 0) return;
    const chatId = this.sessionChatIds.get(sessionId);
    if (!chatId) return;
    const merged = relayData.toolNotifyBuffer.join("\n");
    relayData.toolNotifyBuffer = [];
    relayData.toolNotifyTimer = null;
    this.deps.sendQueue.enqueue(chatId, merged);
    relayData.lastUserFacingMessageTs = Date.now();
  }

  startHeartbeat(sessionId: string, chatId: string): void {
    const relayData = this.sessionRelayData.get(sessionId);
    if (!relayData) return;
    ...
    relayData.heartbeatTimer = setTimeout(() => {
      this.fireHeartbeat(sessionId, chatId);
    }, HEARTBEAT_INITIAL_DELAY_MS);
  }

  private fireHeartbeat(sessionId: string, chatId: string): void {
    const relayData = this.sessionRelayData.get(sessionId);
    if (!relayData) return;
    ...
    const toolHint = relayData.lastActiveToolName
      ? ` (${relayData.lastActiveToolName})`
      : "";
    this.deps.sendQueue.enqueue(chatId, `⏳ 处理中${toolHint} · 已用时 ${timeStr}`);
  ...
  private async handlePermissionRequest(
    ...
    const userSession = sessionManager.getUserSession(userId);
    if (!userSession) {
      console.warn(`[feishu] No userSession for userId=${userId}, sessionId=${sessionId} — ${context} dropped`);
      return;
    }

    console.log(`[feishu] Handling ${context} for session ${sessionId.slice(0, 8)}`);
    ...
    if (settings.feishuAutoApproveSafe && !isDangerousTool(perm.tool_name, perm.input)) {
      ...
      sendQueue.enqueue(chatId, formatted ? `✅ 自动批准 · ${agentLabel}${formatted}` : `✅ 自动批准 · ${agentLabel}${perm.tool_name}`);
    } else if (settings.feishuForwardDangerous) {
      userSession.pendingPermissions.set(perm.request_id, {
        requestId: perm.request_id,
        sessionId,
        toolName: perm.tool_name,
      ...
      sendQueue.enqueue(chatId, `${agentLabel}${formatPermissionRequest(perm.tool_name, perm.input, perm.description)}`, true);
    } else {
      wsBridge.injectPermissionResponse(sessionId, perm.request_id, "allow", perm.input);
    }
  }
